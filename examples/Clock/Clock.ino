...
{
    time32_t utc = now();
    time32_t local = myTZ.toLocal(utc, &tcr);
    Serial.println();
...
// Function to return the compile date and time as a time32_t value
time32_t compileTime()
{
    const time32_t FUDGE(10);     // fudge factor to allow for compile time (seconds, YMMV)
