...
{
    const time32_t n(3);      // number of times to print before and after the time change
    tmElements_t tm;
    tm.Hour = h;
    tm.Minute = 0;
    tm.Second = 0;
    ...
    time32_t utc = makeTime(tm) - offset * SECS_PER_MIN - n;
    ...
    for (uint16_t i=0; i<n*2; i++)
    {
        ...
        time32_t local = tz.toLocal(utc, &tcr);
...
// format and print a time32_t value, with a time zone appended.
void printDateTime(time32_t t, const char *tz)
{
    char buf[32];
