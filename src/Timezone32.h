...
#include <WProgram.h> 
#endif
#include <TimeLib32.h>    // https://github.com/RafaelReyesCarmona/Time32
...
class Timezone
{
    ...
    private:
        ...
        time32_t toTime_t(TimeChangeRule r, int yr);
        ...
        time32_t m_dstUTC;        // dst start for given/current year, given in UTC
        time32_t m_stdUTC;        // std time start for given/current year, given in UTC
        time32_t m_dstLoc;        // dst start for given/current year, given in local time
        time32_t m_stdLoc;        // std time start for given/current year, given in local time
};
#endif
