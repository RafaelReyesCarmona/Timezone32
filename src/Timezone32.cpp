 ...
 *----------------------------------------------------------------------*/
time32_t Timezone::toLocal(time32_t utc)
{
 ...
 *----------------------------------------------------------------------*/
time32_t Timezone::toLocal(time32_t utc, TimeChangeRule **tcr)
{
    // recalculate the time change points if needed
    if (year(utc) != year(m_dstUTC)) calcTimeChanges(year(utc));
 ...
 * should be avoided!                                                   *
 *----------------------------------------------------------------------*/
time32_t Timezone::toUTC(time32_t local)
{
    // recalculate the time change points if needed
...
/*----------------------------------------------------------------------*
 * Determine whether the given UTC time32_t is within the DST interval    *
 * or the Standard time interval.                                       *
 *----------------------------------------------------------------------*/
bool Timezone::utcIsDST(time32_t utc)
{
    // recalculate the time change points if needed
    if (year(utc) != year(m_dstUTC)) calcTimeChanges(year(utc));
...
/*----------------------------------------------------------------------*
 * Determine whether the given Local time32_t is within the DST interval  *
 * or the Standard time interval.                                       *
 *----------------------------------------------------------------------*/
bool Timezone::locIsDST(time32_t local)
{
    // recalculate the time change points if needed
    if (year(local) != year(m_dstLoc)) calcTimeChanges(year(local));
 ...
 * given year as local and UTC time32_t values.                           *
 *----------------------------------------------------------------------*/
 ...
 * for the given year.                                                  *
 *----------------------------------------------------------------------*/
time32_t Timezone::toTime_t(TimeChangeRule r, int yr)
{
